...
pub fn analyze_semantic_rules(
...
) -> Result<(String, AnalyzerOutput), Vec<Box<dyn MainstageErrorExt>>> {
    ...
    for d in diagnostics.drain(..) {
        ...
        let is_error =
            d.level() == crate::error::Level::Error || d.level() == crate::error::Level::Critical;
        if is_error {
            errors.push(d);
        } else {
            non_errors.push(d);
    ...
    if let Some(node) = entrypoint {
                ...
                if let Some(sym) = syms.last() {
                    match sym.kind() {
                        ...
                        crate::analyzers::semantic::symbol::SymbolKind::Function => {
                            analysis
                                .functions
                                .push(crate::analyzers::output::FunctionInfo {
                                    node_id: next_node_id,
        ...
        for scope in &analyzer.get_symbol_table().symbols {
            ...
            for (name, syms) in scope.iter() {
                if let Some(sym) = syms.last() {
                    ...
                    let kind = match sym.kind() {
                        crate::analyzers::semantic::symbol::SymbolKind::Object => {
                            crate::analyzers::output::SymbolKind::Object
                        }
                        crate::analyzers::semantic::symbol::SymbolKind::Function => {
                            crate::analyzers::output::SymbolKind::Function
                        }
                        crate::analyzers::semantic::symbol::SymbolKind::Variable => {
                            crate::analyzers::output::SymbolKind::Variable
                        }
                    };
            ...
            analysis.scopes.push(crate::analyzers::output::ScopeInfo {
                node_id: scope_node,
                parent: None,
                symbols,
            });
            ...
            match node.get_kind() {
                AstNodeKind::Workspace { name, body } => {
                    if let Some(fi) = analysis.objects.iter_mut().find(|o| o.name == *name) {
                        ...
                        collect_from_node(body, Some(nid), analysis);
                    }
                }
                AstNodeKind::Stage { name, args, body } => {
                        ...
                        .find(|f| f.name.as_deref() == Some(name.as_str()))
                    {
                        ...
                        if let Some(args_node) = args.as_ref()
                            && let AstNodeKind::Arguments { args: param_nodes } =
                                args_node.get_kind()
                        {
                ...
                AstNodeKind::Call { callee, args } => {
                    ...
                    for a in args.iter() {
                        if let AstNodeKind::Identifier { name } = a.get_kind() {
                            ...
                            for scope in &mut analysis.scopes {
                                if let Some(sym) =
                                    scope.symbols.iter_mut().find(|s| s.name == *name)
                                {
                                    sym.usages.push((
                                        a.location.clone().unwrap_or_default(),
                                        a.span.clone(),
                                    ));
                                }
                            }
                    ...
                    // traverse args
                    for a in args {
                        collect_from_node(a, current_func, analysis);
                    }
                }
                AstNodeKind::Index {
                    object: array,
                    ...
                    if let AstNodeKind::Identifier { name } = array.get_kind() {
                        for scope in &mut analysis.scopes {
                            if let Some(sym) = scope.symbols.iter_mut().find(|s| s.name == *name) {
                                sym.usages.push((
                                    array.location.clone().unwrap_or_default(),
                                    array.span.clone(),
                                ));
                            }
                        }
                    }
                    collect_from_node(array, current_func, analysis);
                }
                AstNodeKind::Member { object, .. } => {
                    ...
                    if let AstNodeKind::Identifier { name } = object.get_kind() {
                        for scope in &mut analysis.scopes {
                            if let Some(sym) = scope.symbols.iter_mut().find(|s| s.name == *name) {
                                sym.usages.push((
                                    object.location.clone().unwrap_or_default(),
                                    object.span.clone(),
                                ));
                            }
                        }
                    }
                    collect_from_node(object, current_func, analysis);
                }
                AstNodeKind::Script { body } => {
                    for b in body {
                        collect_from_node(b, current_func, analysis);
                    }
                }
                AstNodeKind::Block { statements } => {
                    for s in statements {
                        collect_from_node(s, current_func, analysis);
                    }
                }
                AstNodeKind::If { condition, body } => {
                    collect_from_node(condition, current_func, analysis);
                    collect_from_node(body, current_func, analysis);
                }
                AstNodeKind::IfElse {
                    condition,
                    if_body,
                    ...
                    collect_from_node(condition, current_func, analysis);
                    collect_from_node(if_body, current_func, analysis);
                    collect_from_node(else_body, current_func, analysis);
                }
                AstNodeKind::ForIn { iterable, body, .. } => {
                    collect_from_node(iterable, current_func, analysis);
                    collect_from_node(body, current_func, analysis);
                }
                AstNodeKind::ForTo {
                    initializer,
                    ...
                    collect_from_node(initializer, current_func, analysis);
                    collect_from_node(limit, current_func, analysis);
                    collect_from_node(body, current_func, analysis);
                }
                ...
                AstNodeKind::UnaryOp { expr, .. } => {
                    collect_from_node(expr, current_func, analysis);
                }
                AstNodeKind::BinaryOp { left, right, .. } => {
                    collect_from_node(left, current_func, analysis);
                    collect_from_node(right, current_func, analysis);
                }
                AstNodeKind::Assignment { target, value } => {
                    collect_from_node(target, current_func, analysis);
                    collect_from_node(value, current_func, analysis);
                }
                _ => {}
            }
        }
        ...
        if let Some(mmap) = manifests {
            ...
            fn collect_imports(node: &crate::ast::AstNode, imports: &mut Vec<(String, String)>) {
                ...
                match node.get_kind() {
                    ...
                    AstNodeKind::BinaryOp { left, right, .. } => {
                        collect_imports(left, imports);
                        collect_imports(right, imports);
                    }
                    _ => {}
                }
            }
                ...
                if is_star {
                    ...
                    if let Some(desc) = mmap.get(&module) {
                        ...
                        for f in &desc.manifest.functions {
                            ...
                            analysis.plugin_func_mappings.push((
                                bare,
                                plugin_name.clone(),
                                qualified,
                            ));
                        ...
                        log::warn!(
                            "analyzer: no manifest descriptor found for star import module '{}'",
                            module
                        );
                    }
                } else {
                    ...
                    if let Some(desc) = mmap.get(&module) {
                        ...
                        analysis
                            .plugin_aliases
                            .push((alias.clone(), plugin_name.clone()));
                        for f in &desc.manifest.functions {
                            let qualified = if let Some(dom) = &f.domain {
                                format!("{}.{}", dom, f.name)
                            } else {
                        ...
                        log::warn!(
                            "analyzer: no manifest descriptor found for alias import module '{}'",
                            module
                        );
                    }
                }
            ...
            log::debug!(
                "analyzer: total plugin function mappings: {}",
                analysis.plugin_func_mappings.len()
            );
        }

        // Start traversal from script root
        collect_from_node(ast, None, &mut analysis);
        ...
        fn collect_workspaces(
            n: &crate::ast::AstNode,
            out: &mut Vec<(usize, Option<String>, Vec<String>)>,
        ) {
            match n.get_kind() {
        ...
        if workspaces.is_empty() {
            return Err(vec![Box::new(err::SemanticError::with(
                crate::error::Level::Error,
                "No workspaces found in script; analyzer requires at least one workspace."
                    .to_string(),
        ...
        Err(vec![Box::new(err::SemanticError::with(
            crate::error::Level::Error,
