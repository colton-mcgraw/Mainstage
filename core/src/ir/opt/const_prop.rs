...
pub(crate) fn const_prop(ir: &mut IrModule) {
        ...
        match &op {
            ...
            | IROp::Or { dest, src1, src2 } => {
                ...
                if let (Some(a), Some(b)) = (v1, v2)
                    && let Some(res) = super::compute_binop(&op, &a, &b)
                {
                    // replace with LConst
                    let lc = IROp::LConst {
                        dest: *dest,
            ...
            IROp::GetProp { dest, obj, key } => {
                if let Some(objv) = consts.get(obj).cloned()
                    && let Value::Object(map) = objv
                    && let Some(keyv) = consts.get(key).cloned()
                    && let Value::Symbol(k) = keyv
            ...
            IROp::ArrayGet { dest, array, index } => {
                if let Some(arrv) = consts.get(array).cloned()
                    && let Value::Array(elems) = arrv
                    && let Some(idxv) = consts.get(index).cloned()
                    && let Value::Int(i) = idxv
                {
            ...
            // keep the op.
            _ => {
                if let IROp::LLocal { dest, .. } = &op {
                    consts.remove(dest);
                } else if let IROp::AllocClosure { dest } = &op {
                    consts.remove(dest);
                } else if let IROp::CLoad { dest, .. } = &op {
                    consts.remove(dest);
                } else if let IROp::ArrayNew { dest, .. } = &op {
                    consts.remove(dest);
                } else if let IROp::CallLabel { dest, .. } = &op {
                    consts.remove(dest);
                } else if let IROp::PluginCall { dest: Some(d), .. } = &op {
                    consts.remove(d);
                } else if let IROp::LoadGlobal { dest, .. } = &op {
                    consts.remove(dest);
                } else if let IROp::ArrayGet { dest, .. } = &op {
                    consts.remove(dest);
                } else if let IROp::GetProp { dest, .. } = &op {
                    consts.remove(dest);
                } else if let IROp::Inc { dest } = &op {
                    consts.remove(dest);
                } else if let IROp::Dec { dest } = &op {
                    consts.remove(dest);
                }
                new_ops.push(op);
            }
    ...
    for op in new_ops.iter() {
        match op {
            ...
            IROp::BrTrue { cond, .. } | IROp::BrFalse { cond, .. } => {
                used.insert(*cond);
            }
            IROp::CallLabel {
                dest: _,
