...
impl ExternalPlugin {
    ...
    fn value_to_json(v: &Value) -> serde_json::Value {
        match v {
            ...
            Value::Array(a) => {
                serde_json::Value::Array(a.iter().map(Self::value_to_json).collect())
            }
            Value::Object(m) => {
    ...
    fn json_to_value(j: &serde_json::Value) -> Value {
        match j {
            ...
            serde_json::Value::Array(a) => {
                Value::Array(a.iter().map(Self::json_to_value).collect())
            }
...
impl Plugin for ExternalPlugin {
    ...
    async fn call(&self, func: &str, args: Vec<Value>) -> Result<Value, String> {
        ...
        let mut child = cmd
            .spawn()
            .map_err(|e| format!("spawn plugin '{}': {}", self.exe.display(), e))?;
        ...
        if !output.status.success() {
            ...
            if let Some(j) = maybe_json
                && let Some(err) = j.get("error").and_then(|v| v.as_str())
            {
                return Err(format!("{} (plugin stderr: {})", err, stderr));
            }
            return Err(format!(
                "plugin '{}' exit code: {}\nstdout: {}\nstderr: {}",
                self.name, output.status, stdout, stderr
            ));
        }
