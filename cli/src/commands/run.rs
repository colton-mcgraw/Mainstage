...
pub fn handle(
    ...
    if let Some(user_stl) = matches.get_many::<String>("stl") { stl_selection.extend(user_stl.map(|s| s.to_string())); }
    else if !no_stl { stl_selection.extend(["stdlib".to_string()]); }
        ...
        if let Some(desc) = manifests.get(mod_name) {
            if let Some(dir) = &desc.path {
                ...
                if !desc.manifest.path.trim().is_empty() {
                    ...
                    } else { exe_candidates.push(mp_resolved.clone()); }
                }
                ...
                for c in exe_candidates.into_iter() { match std::fs::metadata(&c) { Ok(meta) if meta.is_file() => { found = Some(c); break; }, _ => continue } }
                ...
                } else { warn!("could not locate executable for plugin module '{}' at expected path(s)", mod_name); }
            } else { warn!("no path specified in manifest for imported module '{}'", mod_name); }
        } else { warn!("no plugin descriptor found for module '{}'", mod_name); }
    };

    for name in stl_selection.iter() { register_by_name(name, name); }
    ...
    for (alias, mod_name) in import_aliases.into_iter() {
        ...
        } else { warn!("no plugin descriptor found for imported module '{}'", mod_name); }
    }

    if let Some(parent) = script.path.parent() { if let Err(e) = std::env::set_current_dir(parent) { warn!("failed to set working dir to {:?}: {}", parent, e); } }

    match run_vm.run(trace) {
        Ok(()) => {}
        Err(e) => { error!("{}", e.lines().collect::<Vec<&str>>().join("\n\t")); }
    }
